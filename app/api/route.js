...
export async function POST(request) {
  ...
  try {
    ...
    // Return a 200 OK response if email is sent successfully
    return new Response(JSON.stringify({ success: true }), { status: 200 });
  } catch (err) {
    // Return a 400 Bad Request response with error message if sending email fails
    console.error(err);
    return new Response(JSON.stringify({ message: err.message }), { status: 400 });
  }
}

// Define function to generate email content
